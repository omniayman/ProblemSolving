...
vector<int> matchingStrings(vector<string> stringList, vector<string> queriesSize) {
    int slistSize=stringList.size();
    int qqueriesSize=queriesSize.size();
    vector<int> matchingString;
    for(int j=0;j<qqueriesSize;j++)
    {matchingString.push_back(0);
    for(int i=0;i<slistSize;i++)
    {
        if(queriesSize[j]==stringList[i])
            matchingString[j]+=1;
